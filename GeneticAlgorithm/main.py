...
if __name__ == '__main__':
    ...
    th.N_RUNS       = config.get("n_runs",    th.N_RUNS)
    th.COMPILER_EXE = config.get("compiler",  th.COMPILER_EXE)
    ...
    if args.benchmark:
        benchmarks_to_run = [{
            ...
            "path"         : args.benchmark,
            "includes"     : [os.path.dirname(args.benchmark) or '.'],
            "polybench"    : False,
            "extra_sources": [],
            "defines"      : [],
        }]
    elif config.get("benchmark_path"):
        bp = config["benchmark_path"]
        benchmarks_to_run = [{
            "name"         : os.path.splitext(os.path.basename(bp))[0],
            "path"         : bp,
            "includes"     : [os.path.dirname(bp) or '.'],
            "polybench"    : False,
            "extra_sources": [],
            "defines"      : [],
        }]
    ...
    for benchmark in benchmarks_to_run:
        ...
        baseline = get_baseline_runtime(
            ...
            extra_sources   = benchmark['extra_sources'],
            extra_includes  = benchmark['includes'],
            defines         = benchmark['defines'],
        )
